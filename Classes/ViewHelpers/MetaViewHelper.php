...
class MetaViewHelper extends \TYPO3\CMS\Fluid\Core\ViewHelper\AbstractViewHelper
{
    ...
    {
        if (!isset($GLOBALS['SUDHAUS7_META_REGISTRY'])) {
            $GLOBALS['SUDHAUS7_META_REGISTRY'] = array();
        }
        self::handler($GLOBALS['SUDHAUS7_META_REGISTRY'], $key, $value, $auto);
        return;
    }

    public static function handler(&$a, $key, $value, $auto)
    {
        if (substr($value, 0, 4)=='EXT:') {
            $value = GeneralUtility::getFileAbsFileName($value);
            $value = str_replace(PATH_site, '', $value);
            if (substr($value, 0, 4)!='http') {
                $value = ($_SERVER['HTTPS'] ? 'https://' : 'http://') . $_SERVER['HTTP_HOST'] . '/' .$value;
            }
        }
        if ($auto) {
            if (isset(self::$plain[$key])) {
                $a[self::$plain[$key]] = $value;
            }
            if (isset(self::$facebook[$key])) {
                $a[self::$facebook[$key]] = $value;
            }
            if (isset(self::$twitter[$key])) {
                $a[self::$twitter[$key]] = $value;
            }
            if (isset(self::$gplus[$key])) {
                $a[self::$gplus[$key]] = $value;
            }
        } else {
            $a[$key] = $value;
        }
