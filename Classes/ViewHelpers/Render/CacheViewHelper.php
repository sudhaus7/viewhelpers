...
/*
 * Inspiriert von vhs:render.cache
 */
...
class CacheViewHelper extends AbstractRenderViewHelper {
    const ID_PREFIX = 's7-render-cache-viewhelper';
    const ID_SEPARATOR = '-';
    /**
    ...
    public function initialize() {
        ...
        $this->cache = $cacheManager->getCache('sudhaus7viewhelpers_metatags');
    }
    /**
     * Render
     *
    ...
    protected function store($value, $id) {
        $this->cache->set(self::ID_PREFIX . self::ID_SEPARATOR . $id, $value,array('pageId_' . $GLOBALS['TSFE']->id),3600);
    }
    /**
     * @param string $id
     * @return mixed
